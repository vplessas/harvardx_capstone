...
For that reason, a significant part of this report has been devoted in exploratory data analysis and in time-series features analysis.
Due to the large number of independent time series in the dataset (over three thousand product codes), we have decided to group our data in higher granularity product categories based on a two dimensional 80/20 rule.
The new product hierarchy consisted of:
...
We have used those features to fine tune the algorithm's used during the development of our model.
...
Our methodology commenced by splitting our data into a 28 days long dataset named online_store_validate which we'll be reserved until the very end of our analysis to test our model's accuracy, and a fit dataset named online_store_fit which contains all data prior to the validation set.
...
The algorithm with the best performance we'll be then recalculated over the entire online_store_fit dataset and produce 28 days of multi-step forecasts to compare with the actual values in the validation set.
...
We have provided a visual representation of the forecasts generated from the fitted values on the train set as well as their 80% and 95% confidence intervals.
...
Exponential Smoothing and specifically the Damped Trend with multiplicative error and seasonality had outperformed all other methods by producing a smaller MAPE = 25% when tested and validated against hold-out part of our dataset reserved for that final estimation.

To ensure we haven't over fitted our model and that is generalised enough to produce unbiased forecasts based on new data we have used timeseries bootstrapping to simulate 100 time series and produced individual forecasts using the Exponential Smoothing algorithm [@BERGMEIR2016303].
...
The algorithms tested where a) The naive method (SNAIVE) with a trend and seasonal component applied, b) Linear Regression (TSLM) with trend and seasonality, and c) the Exponential Smoothing algorithm in its multiple versions for trend and seasonality adjustments.
...
With that in mind there's further analysis that coud potentially improve prediction on this dataset and which we haven't included in this instance.
...
Additionally, our dataset was a multivariate one which we coerced into a univariate time series.
More advanced machine learning techniques such as Non-Liner Regression and clustering could have provided further insights on the interaction between variables in the dataset as well as providing more sophisticated classification of product and potentially customer groups.
